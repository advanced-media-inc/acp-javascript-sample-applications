// opus recorderを利用するオプションを追加
// AudioWorklet.addModule()で設定するスクリプトを外部ファイル化(Chrome拡張機能から使用するのでchrome.runtime.getURL()で指定)
// WebSocketでもwavファイルを作成するよう変更 → Chrome拡張機能では使用しないので削除
// システム音も録音できるよう変更(getDisplayMedia()を使用)
// 録音対象を選べるようにuseUserMedia, useDisplayMediaオプションを追加
...
//				if (recorder_.recorded) recorder_.recorded(pcmData_.subarray(pcmDataOffset, pcmDataIndex));
	...
	async function resume_() {
		...
		try {
			...
			if (recorder_.useUserMedia) {
			...
			} else if (recorder_.useDisplayMedia) {
				...
				throw new Error("No recording device specified.");
			}
			audioStream_ = audioStream;
			audioStream_.stopTracks = function () { stopTracks(audioStream_) };
			if (state_ === 3) {
				state_ = 4;
				audioStream_.stopTracks();
				if (audioDecimatationFactor_ > 1) {
			...
			if (recorder_.useUserMedia && recorder_.useDisplayMedia) {
				...
				// マイク音声とシステム音声のミックス
				const audioCtx = new AudioContext({ sampleRate: recorder_.sampleRate });
				...
				mixedStream.addTrack(dest.stream.getTracks()[0]);
				audioStream_ = mixedStream;

				audioStream_.stopTracks = function () {
					stopTracks(audioStream_);
					audioStream.getTracks().forEach(track => track.stop());
...
//			if (recorder_.resumeEnded) recorder_.resumeEnded(((ima_state_ > 0) ? "" : "MSB") + (audioSamplesPerSec_ / 1000 | 0) + "K");
			...
			state_ = 0;
			if (recorder_.TRACE) recorder_.TRACE("ERROR: can't start recording (" + e.message + ")");
			reason_.code = 2;
			reason_.message = e.message;
			if (recorder_.pauseEnded) recorder_.pauseEnded(reason_, waveFile_);

			if (audioStream_ != null && typeof audioStream_.stopTracks !== 'undefined') {
				audioStream_.stopTracks();
			}
		}
